...
def load_dependencia(prefixo: str, subordinada: str):
    ...
    SELECT prefixo, subordinada, nome, uor, condominio, cod_condominio, linha_necessaria,
           ...
           ramais_controle, ramais_manter, pendencias, observacoes, cod_status, lote, concluido
    FROM dependencia
...
# ---------------------- Abas ----------------------
aba_listar, aba_editar, aba_incluir = st.tabs(["📋 Listar", "✏️ Editar", "➕ Incluir"])
...
with aba_listar:
    with st.expander("Filtros", expanded=True):
        ...
        f_somente_nao_concluidas = st.checkbox("Mostrar somente não concluídas", value=False, key="dep_f_nao_concl")

    sql = (
        "SELECT prefixo, subordinada, nome, uor, cod_condominio, "
        "ramais_controle, ramais_manter, cod_status, lote, concluido "
        "FROM dependencia WHERE 1=1"
    )
    params = []
    if f_prefixo.strip():
    ...
    if not rows:
        ...
        if "concluido" in df_view.columns:
            df_view["concluido"] = df_view["concluido"].fillna(0).astype(bool)
        df_view["Concluir ciclo"] = False
        ...
        edited = st.data_editor(
            ...
            column_config={
                "concluido": st.column_config.CheckboxColumn(
                    "Concluído?",
                    help="Situação atual do registro",
                    disabled=True,
                ),
                "Concluir ciclo": st.column_config.CheckboxColumn(
                    label="Concluir ciclo",
                    help="Marque para definir concluído=TRUE (1)",
                    default=False,
                ),
                "Excluir": st.column_config.CheckboxColumn(
                    label="Excluir",
                    help="Marque para excluir fisicamente o registro",
                    default=False,
                ),
            },
        )

        to_conclude = edited.loc[edited["Concluir ciclo"] == True, ["prefixo", "subordinada"]] if "Concluir ciclo" in edited.columns else pd.DataFrame(columns=["prefixo","subordinada"])
        to_delete = edited.loc[edited["Excluir"] == True, ["prefixo", "subordinada"]] if "Excluir" in edited.columns else pd.DataFrame(columns=["prefixo","subordinada"])

        overlap = set(map(tuple, to_conclude.values)) & set(map(tuple, to_delete.values))
        if overlap:
            st.info(f"{len(overlap)} registro(s) marcados simultaneamente para Concluir e Excluir — a exclusão prevalecerá.")

        c1, c2, c3 = st.columns([1,1,2])
        with c1:
            btn_conc = st.button(f"✅ Concluir processo ({len(to_conclude)})", disabled=len(to_conclude) == 0)
        with c2:
            btn_del = st.button(f"🗑️ Excluir selecionados ({len(to_delete)})", disabled=len(to_delete) == 0)
        with c3:
            st.caption("Marque nas colunas e clique no botão correspondente.")

        if btn_conc and len(to_conclude) > 0:
            try:
                for pfx, sub in map(tuple, to_conclude.values):
                    if (pfx, sub) in overlap:
                        continue
                    execute("UPDATE dependencia SET concluido=1 WHERE prefixo=%s AND subordinada=%s", (pfx, sub))
                st.success(f"Concluído para {len(to_conclude) - len(overlap)} registro(s).")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
                st.error(f"Erro ao concluir: {e}")

        if btn_del and len(to_delete) > 0:
            try:
                for pfx, sub in map(tuple, to_delete.values):
                    execute("DELETE FROM dependencia WHERE prefixo=%s AND subordinada=%s", (pfx, sub))
                st.success(f"Excluídos {len(to_delete)} registro(s).")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
...
with aba_editar:
    ...
    with c1:
        prefixo_ed = st.text_input("Prefixo (4)", max_chars=4, key="dep_ed_prefixo", label_visibility="collapsed", placeholder="0000")
    with c2:
        subordinada_ed = st.text_input("Subordinada (2)", max_chars=2, key="dep_ed_subordinada", label_visibility="collapsed", placeholder="00")
    with c3:
    ...
    with c4:
        ...
        if (prefixo_ed and len(prefixo_ed.strip()) == 4) and (subordinada_ed and len(subordinada_ed.strip()) == 2):
            ...
            if nome_badge:
                st.markdown(f"<div style='padding:6px 10px;background:#F0F2F6;border-radius:6px;display:inline-block'>🏷️ <b>{prefixo_ed}-{subordinada_ed}</b> — {nome_badge}</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='padding:6px 10px;background:#FFF3CD;border-radius:6px;display:inline-block'>❓ <b>{prefixo_ed}-{subordinada_ed}</b> — não encontrada</div>", unsafe_allow_html=True)
    ...
    if not data:
        ...
        with r2c3:
            st.selectbox("Status", status_options, index=status_options.index(status_initial) if status_initial in status_options else 0, key="dep_vw_status")
        ...
        with colC:
            already_done = bool(data.get('concluido'))
            concluir_clicked = st.button("✅ Concluir processo", key="dep_ed_btn_concluir", disabled=already_done, help="Define concluído=TRUE (1)")
        ...
        if concluir_clicked:
            try:
                execute("UPDATE dependencia SET concluido=1 WHERE prefixo=%s AND subordinada=%s", (prefixo_ed.strip(), subordinada_ed.strip()))
                st.success("Processo concluído para a dependência.")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
                st.error(f"Erro ao concluir processo: {e}")

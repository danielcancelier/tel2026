...
from db import fetch_all, fetch_one  # usa suas funções utilitárias de conexão/consulta
...
def load_status_lookup():
    rows = fetch_all("SELECT codigo, descricao FROM status ORDER BY descricao")
    lut = {r["codigo"]: (r["descricao"] or "Sem descrição") for r in rows}
    return lut

@st.cache_data(ttl=60, show_spinner=False)
def load_dependencias():
    sql = """
        SELECT d.prefixo, d.subordinada, d.cod_status, d.lote,
...
def load_atualizacoes_all():
    sql = """
        SELECT a.prefixo, a.subordinada, a.cod_produto, p.descricao AS produto,
               a.cod_status, s.descricao AS status_desc, a.quantidade,
               a.data_status
          FROM atualizacoes a
...
with st.sidebar:
    ...
    st.subheader("✅ Status considerados 'Concluídos'")
    # Sugestão automática a partir das descrições existentes
    descrs = sorted(set([d for d in status_lut.values() if d]))
    sugestao = [d for d in descrs if any(x in d.lower() for x in
                                         ["conclu", "final", "encerr", "implant"])]
    concluidos = st.multiselect(
        "Escolha 1+ descrições de status",
        options=descrs,
        default=sugestao,
        help="Esses status serão usados para % de concluídos."
    )
...
dep_concluidas = int((df_dep["status_desc"].fillna("")).isin(concluidos).sum())
...
df_at_ref = df_at_last if modo_at == "Último por dependência/produto" else df_at_all
recentes = 0
if not df_at_all.empty:
    recentes = int(df_at_all.loc[df_at_all["data_status"].notna() &
                                 (pd.to_datetime(df_at_all["data_status"]) >= pd.to_datetime(janela))].shape[0])
...
st.caption(
    "KPIs calculados a partir das tabelas `dependencia` e `atualizacoes` e da lista de `status` definidos no seu banco. "
    "As funções de acesso a dados vêm do seu `db.py`."  # explicação
)
...
with tab_dep:
    ...
    if not df_dep.empty:
        df_dep_plot = (df_dep
                       .assign(status_desc=lambda d: d["status_desc"].fillna("Sem status"))
        ...
        if "lote" in df_dep.columns and df_dep["lote"].notna().any():
            ...
            df_tot_lote = df_lote.groupby("lote", as_index=False)["size"].sum().rename(columns={"size": "tot_lote"})
...
with tab_at:
    ...
    if not df_src.empty:
        ...
        with colA:
            ...
            df_by_status = (df_src.groupby("status_desc", as_index=False)
                                  .size().rename(columns={"size": "total"}))
            chart_stat = alt.Chart(df_by_status).mark_arc(innerRadius=60).encode(
                theta="total:Q",
                color=alt.Color("status_desc:N", legend=alt.Legend(title="Status")),
                tooltip=[alt.Tooltip("status_desc:N", title="Status"),
                         alt.Tooltip("total:Q", title="Qtde", format=",.0f")]
            ).properties(height=320)
            st.altair_chart(chart_stat, use_container_width=True)

        with colB:
            st.caption("Top produtos por concluídos")
            df_conc = df_src[df_src["status_desc"].isin(concluidos)]
            if not df_conc.empty:
                df_top = (df_conc.groupby("produto", as_index=False)
                                  .size().rename(columns={"size": "concluidos"})
                                  .sort_values("concluidos", ascending=False)
                                  .head(15))
                chart_prod = alt.Chart(df_top).mark_bar().encode(
                    x=alt.X("concluidos:Q", title="Concluídos"),
                ...
                st.info("Nenhum registro está nos status marcados como 'Concluídos'.")
...
with tab_time:
    st.subheader("Evolução de concluídos no tempo")
    if not df_at_all.empty:
        tmp = df_at_all.copy()
        tmp["data_status"] = pd.to_datetime(tmp["data_status"], errors="coerce")
        tmp = tmp[tmp["data_status"].notna()]
        tmp["is_conc"] = tmp["status_desc"].fillna("").isin(concluidos)
        di = (tmp[tmp["is_conc"]]
        ...
        if not di.empty:
            ...
            chart_time = alt.Chart(di).mark_area(opacity=0.6).encode(
                ...
                y=alt.Y("acum:Q", title="Acumulado (concluídos)"),
                tooltip=[alt.Tooltip("data_status:T", title="Data"),
                         alt.Tooltip("size:Q", title="No dia", format=",.0f"),
                         alt.Tooltip("acum:Q", title="Acumulado", format=",.0f")]
            ).properties(height=380)
            st.altair_chart(chart_time, use_container_width=True)
        else:
            st.info("Não há atualizações em status 'Concluídos' com data preenchida.")
    else:
        st.warning("Não encontrei registros em `atualizacoes`.")

...
# ---------------------- Cache de dependências (mesmo padrão do 2_Atualizações) ----------------------
...
with aba_incluir:
    ...
    if st.button("➕ Incluir", key="com_bt_incluir_tab"):
...
with aba_editar:
    ...
    id_edit = st.text_input("ID (somente para editar ou excluir)", key="com_ed_id")
    ...
    with colA:
        if st.button("➕ Incluir", key="com_bt_incluir_legacy"):
            if id_edit.strip():

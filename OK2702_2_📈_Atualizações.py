...
# ---------------------- Estilos globais (badge, inputs, botão, linha flex) ----------------------
st.markdown(
    """
<style>
/* Badge do nome */
...
.stButton > button[kind="secondary"] { padding:.50rem .55rem; font-size:.95rem; }

/* Linha/colunas da faixa superior */
.atu-row { display:flex; align-items:center; gap:.75rem; flex-wrap:nowrap; }
.atu-col { display:flex; flex-direction:column; }
.atu-col.small { min-width:7rem; max-width:9rem; }
.atu-col.button { min-width:auto; }
.atu-col.badge { min-width:16rem; }
</style>
""",
...
def load_dependencias_for_select():
    """Mantida p/ compatibilidade; a busca principal usa cache_dependencias()."""
    rows = fetch_all(
...
def load_dependencias(prefixo_filter: str = ""):
    """Mantida p/ compatibilidade. Preferir cache_dependencias()."""
...
def cache_dependencias():
    ...
    options = [f"{p}-{s} — {n}" if n else f"{p}-{s}"
...
def dependencia_existe(prefixo: str, subordinada: str):
    """Validação local extremamente rápida (O(1))."""
...
with aba_incluir:
    ...
    # === Linha única com alinhamento rígido (flex) ===
    # Ajuste fino de larguras nos pesos abaixo [1,1,0.25,2] e/ou nas min-width do CSS (.atu-col.small/.atu-col.badge)
    col1, col2, col3, col4 = st.columns([1, 1, 0.25, 2], vertical_alignment="center")
    with col1:
        st.markdown("<div class='atu-col small'>", unsafe_allow_html=True)
        sel_prefixo = st.text_input("Prefixo (4)", max_chars=4, key="atu_inc_prefixo")
        st.markdown("</div>", unsafe_allow_html=True)

    with col2:
        st.markdown("<div class='atu-col small'>", unsafe_allow_html=True)
        sel_sub = st.text_input("Subordinada (2)", max_chars=2, key="atu_inc_sub")
        st.markdown("</div>", unsafe_allow_html=True)

    with col3:
        st.markdown("<div class='atu-col button'>", unsafe_allow_html=True)
        ...
        if st.button("🔍", key="atu_inc_btn_buscar", type="secondary"):
            abrir_busca_dependencia()

        st.markdown("</div>", unsafe_allow_html=True)

    with col4:
        st.markdown("<div class='atu-col badge'>", unsafe_allow_html=True)
        # Badge do nome (instantâneo ao completar 4+2)
...
with aba_editar:
    ...
    # Mantém a mesma ideia de linha compacta para coerência visual
    e1, e2, e3, e4 = st.columns([1, 1, 0.25, 2], vertical_alignment="center")
    with e1:
        prefixo = st.text_input("Prefixo (4)", max_chars=4, key="atu_ed_prefixo")
    with e2:
        subordinada = st.text_input("Subordinada (2)", max_chars=2, key="atu_ed_sub")
    with e3:
        # Placeholder do botão (se quiser, replico o modal aqui também)
